...
public class new_comment {
    ...
    public String getCommentUserHead()
    {
        ...
        if (commentUserHead!= null && commentUserHead.startsWith("http"))
             {
                 commentUserHead=commentUserHead;
        } else if (commentUserHead.startsWith("uploads/minhader")) {
            commentUserHead= ComantUtils.MyUrlHot1+commentUserHead;
        }
        else {
         commentUserHead=AppConfig.sRootUrl+"/logincontroller/getHeadImg/"+commentUserHead;
        }
