...
public class RequestListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.tv_list_state.setSelected(true);
        holder.tv_list_state.setTextColor(R.color.request_color);
        if (requestListInfos.get(position).getUserId().equals(MyApplication.getCurrentUserInfo().getUserId())) {//作为请求方
            ...
            if (requestListInfos.get(position).getRequestState().equals("1")) {
               ...
               holder.tv_list_state.setSelected(true);
                holder.tv_list_state.setTextColor(R.color.request_color);
                holder.tv_list_data.setText("ID : "+requestListInfos.get(position).getRequestUserId() + "用户成为您的好友");
            } else {
    ...
    private void dealRequest(String requestId) {
        ReqDealRequest.dealRequest(context, "adapter", MyApplication.getCurrentUserInfo().getUserId(), requestId, 2 + "", new EntityCallBack<BaseResp>() {
            @Override
            ...
            public void onSuccess(BaseResp resp) {
                if (resp.is200()) {
                    MyToast.show(context, "成功添加对方为好友，你们现在可以聊天啦");
                    freshListener.fresh();
                }
            }
